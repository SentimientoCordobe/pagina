...
export default function Home() {
  // Las constantes se definen aquí, fuera del return
      ...
      <div className="grid gap-8 lg:grid-cols-[320px_1fr_320px]">
        ...
        <aside className="space-y-6">
          ...
          {videoPrincipalId && (
            <div className="space-y-2">
              {/* Opcional: El vídeo fijo que tenías arriba */}
          ...
          {videos.slice(1).map((v) => (
            <div key={v.id} className="space-y-2">
              ...
              <h3 className="text-sm font-semibold leading-tight">{v.titulo}</h3>
        ...
        <main className="space-y-8">
          {noticias.map((n) => (
            ...
            >
              <img src={n.imagen} alt={n.titulo} className="h-60 w-full object-cover" />
              <div className="p-6">
                <h2 className="text-2xl font-bold leading-snug">{n.titulo}</h2>
                <p className="mt-3 text-muted-foreground">{n.resumen}</p>
              </div>
            </article>
          ))}
        </main>
              ...
              <tbody>
                {clasificacion.map((e) => (
                  <tr key={e.posicion} className="border-t">
                    ...
                    <td className="p-2 text-right font-semibold">{e.puntos}</td>
                  </tr>
                ))}
              </tbody>
            </table>
